...
function ReturnCell({ value }: { value: number | null }) {
  if (value === null || value === undefined) {
    return (
      <div className="px-3 py-1.5 text-xs font-semibold text-center w-[120px]" style={{ color: '#6b7280' }}>
        --
      </div>
    );
  }
  ...
  return (
    <div className="px-3 py-1.5 text-xs font-semibold text-center w-[120px]" style={{ color: textColor }}>
      {formatted}
    </div>
  );
}
...
export function TopBottomFactors({ timeFrame }: TopBottomFactorsProps) {
  ...
  // Get bottom 5 factors (worst performers)
